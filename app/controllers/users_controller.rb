class UsersController < ApplicationController
 # GET /users
  ...
  def create
    ...
    respond_to do |format|
      if @user.save
        format.html { redirect_to(@user, :notice => 'De gebruiker was succesvol aangemaakt.') }
  ...
  def update
    ...
    respond_to do |format|
      if @user.update_attributes(params[:user])
        format.html { redirect_to(@user, :notice => 'De gebruiker was succesvol geupdate.') }
        format.xml  { head :ok }
      else
...
def authenticate
		...
		@user = User.new(params[:usersform])
        ...
        #redirects the user to our private page.
			...
			flash[:notice] = "Foute naam en/of Paswoord"
